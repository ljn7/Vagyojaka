    ...
    : QAbstractTableModel(parent)
{
    OriginalData empty;
    empty.words = "";
    empty.notPronouncedProperly = "";
    empty.tags = "";
    m_originalData[-1] = empty;
}
...
//         row.wordsEdited = value.toBool();
//         emit dataChanged(index, index, {role});
//         return true;
//     }
//     if (role == Qt::UserRole + 2) {
//         row.pronunciationEdited = value.toBool();
//         emit dataChanged(index, index, {role});
//         return true;
//     }
//     if (role == Qt::UserRole + 3) {
//         row.tagEdited = value.toBool();
//         emit dataChanged(index, index, {role});
//         return true;
//     }
...
//         switch (index.column()) {
//             case 1:
//                 row.words = value.toString();
//                 break;
//             case 2:
//                 row.not_pronounced_properly = value.toString();
//                 break;
//             case 3:
//                 row.tag = value.toString();
//                 break;
//             case 4:
//                 row.sound_quality = value.toInt();
//                 break;
//             case 5:
//                 row.asr_quality = value.toInt();
//                 break;
//             default:
//                 return false;
//         }
...
const OriginalData& LazyLoadingModel::getOriginalData(int row) const {
    ...
    if (it == m_originalData.end()) {
        return m_originalData.find(-1).value();  // Return empty data
    }
    return it.value();
}
...
bool LazyLoadingModel::setData(const QModelIndex& index, const QVariant& value, int role) {
    ...
    //     // Handle edit tracking for text columns
    //     if (column >= 1 && column <= 3) {
    //         QString delimiter = (column == 3) ? ";" : " ";
    //         // updateEditHistory(row, column, newText, delimiter);
    //         const OriginalData& originalData = getOriginalData(row);
    ...
    //         int wordDiff = calculateChangedWords(newText, originalText, delimiter);

    //         // Determine if this edit reverts to original
    //         // bool isReverted = isRevertedToOriginal(row, column, newText);

    //         bool isReverted = (newText == originalText);


    //         // // Update background color and edited flag
    //         // if (isReverted) {
    ...
    //         //     auto eCount = editedCounts[row];
    //         //     int diff = 0;
    //         //     switch (column) {
    //         //     case 1:
    //         //         diff = calculateChangedWords(newText, getOriginalData(row).words, delimiter);
    //         //         if (diff != eCount.editedWords) {
    //         //             emit transcriptEditedWordsCount(diff);
    //         //             eCount.editedWords = diff;
    //         //         }
    //         //         break;
    //         //     case 2:
    //         //         diff = calculateChangedWords(newText, getOriginalData(row).notPronouncedProperly, delimiter);
    //         //         if (diff != eCount.editedNotPronouncedProperlyWords) {
    //         //             emit mispronouncedEditedWordsCount(diff);
    //         //             eCount.editedNotPronouncedProperlyWords = diff;
    //         //         }
    //         //         break;
    //         //     case 3:
    //         //         diff = calculateChangedWords(newText, getOriginalData(row).tags, delimiter);
    //         //         if (diff != eCount.edittedTaggedWords) {
    //         //             emit taggedEditedWordsCount(diff);
    //         //             eCount.edittedTaggedWords = diff;
    //         //         }
    //         //         break;
    //         //     }
    //         //     editedCounts[row] = eCount;
    //         // }
    ...
    //             switch (column) {
    //             case 1: emit transcriptEditedWordsCount(-wordDiff); break;
    //             case 2: emit mispronouncedEditedWordsCount(-wordDiff); break;
    //             case 3: emit taggedEditedWordsCount(-wordDiff); break;
    //             }
    //         } else {
    //             setData(index, QBrush(Qt::yellow), Qt::BackgroundRole);
    //             setData(index, true, Qt::UserRole + column);
    ...
    //     // Continue with existing setData logic
    //     TTSRow& trow = m_rows[index.row()];
    //     switch (index.column()) {
    //         case 1:
    //             trow.words = newText;
    //             break;
    //         case 2:
    //             trow.not_pronounced_properly = newText;
    //             break;
    //         case 3:
    //             trow.tag = newText;
    //             break;
    //         case 4:
    //             trow.sound_quality = value.toInt();
    //             break;
    //         case 5:
    //             trow.asr_quality = value.toInt();
    //             break;
    //         default:
    //             return false;
    //     }
...
int LazyLoadingModel::calculateChangedWords(const QString& current, const QString& original, const QString& delimiter) const {
    ...
    // // First check if texts are identical to avoid unnecessary calculation
    // if (current == original) {
    //     return 0;
    // }

    // int changedWords = 0;
    // if (currentWords.size() > originalWords.size()) {
    //     changedWords += currentWords.size() - originalWords.size();
    //     for (const QString& word : originalWords) {
    //         if (!currentWords.contains(word)) changedWords++;
    //     }
    // } else if (currentWords.size() < originalWords.size()) {
    //     changedWords += originalWords.size() - currentWords.size();
